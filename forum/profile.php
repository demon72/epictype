		...
		else
		{
			if (isset($_POST['form_sent']))
			{
				...
				if (empty($errors))
				{
					...
					$query = array(
						'UPDATE'	=> 'users',
						'SET'		=> 'password=\''.$new_password_hash.'\', activate_key=NULL'.($user['cp']==1?', cp=0':''),
						'WHERE'		=> 'id='.$id
					);
...
<?php
	...
	if (isset($_POST['form_sent']))
	{
		...
		if (empty($errors))
		{
			...
			$query = array(
				'UPDATE'	=> 'users',
				'SET'		=> 'password=\''.$new_password_hash.'\''.($user['cp']==1?', cp=0':''),
				'WHERE'		=> 'id='.$id
			);
...
<?php if (!$forum_user['is_admmod'] || $forum_user['id'] == $id): ?>
				<div class="sf-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="sf-box text required">
						...
						<span class="fld-input"><input type="<?php echo($forum_config['o_mask_passwords'] == '1' ? 'password' : 'text') ?>" id="fld<?php echo $forum_page['fld_count'] ?>" name="req_old_password" size="35" value="<?php if($user['cp']==1){echo $user['username'];}else{if (isset($_POST['req_old_password'])) echo forum_htmlencode($_POST['req_old_password']);} ?>" required /></span>
					</div>
				</div>
...
else if (isset($_POST['form_sent']))
{
	...
	switch ($section)
	{
		case 'identity':
		{
			$form = extract_elements(array('realname', 'url', 'location', 'jabber', 'icq', 'msn', 'aim', 'yahoo','vk'));
	...
	($forum_user['g_moderator'] != '1' || $forum_user['g_mod_edit_users'] == '0' || $user['g_id'] == FORUM_ADMIN || $user['g_moderator'] == '1'))
{
	...
	if ($user['url'] != '')
	{
		...
		$forum_page['url'] = '<span class="ah" onclick="window.location=\''.$url_source.'\'">'.$user['url'].'</span>';
		//$forum_page['url'] = '<!--noindex--><a href="'.$url_source.'" class="external url" target="_blank" rel="nofollow">'.$user['url'].'</a><!--/noindex-->';
	...
	if ($user['vk'] != '')
	{
		if ($forum_config['o_censoring'] == '1')
		{
			$user['vk'] = censor_words($user['vk']);
		}

		$vk_url = ((strpos($user['vk'], 'http://') === 0) || (strpos($user['vk'], 'https://') === 0)) ?
			forum_htmlencode($user['vk']) :
			forum_htmlencode('https://vk.com/'.$user['vk'])
		;
		$forum_page['vk'] = '<a href="'.$vk_url.'" class="external url">'.$vk_url.'</a>';
		$forum_page['user_contact']['vk'] = '<li><span>'.$lang_profile['VK'].': '.$forum_page['vk'].'</span></li>';
	}// Twitter
	if ($user['twitter'] != '')
	{
		if ($forum_config['o_censoring'] == '1')
	...
	/*if ($user['linkedin'] != '')
	{
		...
		$forum_page['linkedin'] = '<span class="ah" onclick="window.location=\''.$linkedin_url.'\'">'.$linkedin_url.'</span>';
		//$forum_page['linkedin'] = '<!--noindex--><a href="'.$linkedin_url.'" class="external url" target="_blank" rel="nofollow">'.$linkedin_url.'</a><!--/noindex-->';
...
else
{
	...
	if ($section == 'about')
	{
		...
		if ($user['url'] != '')
		{
			...
			$forum_page['user_contact']['website'] = '<!--noindex--><li><span>'.$lang_profile['Website'].': '.$forum_page['url'].'</span></li>';
		}
		...
		/*if ($user['linkedin'] != '')
		{
			...
			$linkedin_url = forum_htmlencode($user['linkedin']);
			//$forum_page['linkedin'] = '<!--noindex--><a href="'.$linkedin_url.'" class="external url" target="_blank" rel="nofollow">'.$linkedin_url.'</a><!--/noindex-->';
			$forum_page['linkedin'] = '<span class="ah" class="ah" onclick="window.location=\''.$linkedin_url.'\'">'.$linkedin_url.'</span>';
			$forum_page['user_contact']['linkedin'] = '<li><span>'.$lang_profile['LinkedIn'].': '.$forum_page['linkedin'].'</span></li>';
		}*/
				...
				<div class="soc_auth">
					...
					<div class="soc_slide">
						<div class="slideThree">	
							<input type="checkbox" value="None" id="slideThree" name="check"  <?php $c=0; if($user['vk_id']>0){echo 'checked';}?>/>
							<label for="slideThree"></label>
							<script>
								function authInfo(response) {
									if(response.session) {
										$.post('/forum/ajax.php',{module:'add',method:'vk',id:<?php echo $user['id'];?>,vk_id:response.session.mid},function(data){
											alert('Страница привязана');
										},'json');
									}
								}
								$(function() {
									...
									$('#slideThree').click(function(){
										status=$('#slideThree').prop("checked");	
										if(status=='false'){
											$.post('/forum/ajax.php',{module:'del',method:'vk',id:<?php echo $user['id'];?>},function(data){
												alert('Страница отвязана');
											},'json');
										}
										if(status=='true'){
