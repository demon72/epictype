...
// Login
if (isset($_POST['form_sent']) && empty($action) && $_POST['confirm']=='true')
{
...
<?php ($hook = get_hook('li_login_group_end')) ? eval($hook) : null; ?>
			...
			<div style="width:310px !important;height:32px;padding:5px;">
				<script src="//vk.com/js/api/openapi.js" type="text/javascript"></script>
				<div class="vk" id="login_button" onclick="VK.Auth.login(authInfo);">&nbsp;</div>
				<script language="javascript">
					...
					function authInfo(response) {
					  if (response.session) {
					    $.post('/forum/ajax.php',{module:'auth',method:'vk',id:response.session.mid},function(data){
					    	location.reload();
					    },'json');
					  }
					}
	...
	<script>
		$(function(){
			$("input[name=login]").mouseenter (function(){
				$("input[name=login]").click(function(){
					$("input[name=req_username]" ).before('<input type="hidden" name="confirm" value="true">');
				});
			});	
			$("input[name=req_username]").keydown(function(e){
				if(e.which==13){
					$("input[name=req_username]" ).before('<input type="hidden" name="confirm" value="true">');
				}
			});
			$("input[name=req_password]").keydown(function(e){
				if(e.which==13){	
					$("input[name=req_username]" ).before('<input type="hidden" name="confirm" value="true">');
				}
			});
