...
$forum_page['sort_by'] = (!isset($_GET['sort_by']) || $_GET['sort_by'] != 'username' && $_GET['sort_by'] != 'registered' && ($_GET['sort_by'] != 'num_posts' || !$forum_page['show_post_count'])) ? 'num_posts' : $_GET['sort_by'];
...
$query = array(
	...
	'WHERE'		=> 'u.id > 1 AND u.group_id != '.FORUM_UNVERIFIED.' and b.id is NULL'
);
...
// Grab the users
$query = array(
	'SELECT'	=> 'u.id, u.username,u.realname, u.title, u.num_posts, u.registered, g.g_id, g.g_user_title',
	'FROM'		=> 'users AS u',
	'JOINS'		=> array(
		array(
	...
	'WHERE'		=> 'u.id > 1 AND u.group_id != '.FORUM_UNVERIFIED .'  and b.id is NULL',
	'ORDER BY'	=> $forum_page['sort_by'].' DESC, registered DESC',
...
<?php
	...
	foreach ($founded_user_datas as $user_data)
	{
		...
		$forum_page['table_row']['username'] = '<td class="tc'.count($forum_page['table_row']).'"><a href="'.forum_link($forum_url['user'], $user_data['id']).'">'.forum_htmlencode(($user_data['realname']?$user_data['realname']:$user_data['username'])).'</a></td>';
